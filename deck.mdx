...
_There are 11 letters between the "a" and the "y"._
...
- A is the easiest to meet, AAA is the most difficult
- At the AAA level, satisfying some criteria may come at the expense of other criteria and may not be feasible.
...
Attributes exist for _**roles**_, _**states**_, and _**properties**_. Examples:
- Role: `role="button"`
...
DOM elements such as buttons, anchors, and inputs effectively have built-in ARIA attributes, so role and state attributes aren't necessary.

Property attributes might still be useful in certain situations, such as providing a label for an input that doesn't have an `<label>` element, or providing text content for an button that only contains an icon.
